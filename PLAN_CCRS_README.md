...
- Observation: Optional Java capability wrappers need their transitive runtime jars on the JPype classpath, not only the CCRS module jars and direct SDK jars.
  Evidence: A 2026-06-06 notebook run with `enable_contingency_llm_prediction=True` and `enable_contingency_a2a_consultation=True` reached `ContingencyCcrs.evaluateWithTrace(...)` after an explicit `escalate_to_contingency_ccrs` call, then failed with `NoClassDefFoundError: com/fasterxml/jackson/core/JsonProcessingException`. Gradle runtime dependency reports for `:ccrs-langchain4j` and `:ccrs-a2a` showed Jackson and other optional-module transitives that were missing from [java_runtime.py](react_agent/ccrs/java_runtime.py). After adding those coordinates, a fresh Python smoke registered `retry`, `backtrack`, `consultation`, `prediction_llm`, and `stop` and returned a normal contingency result instead of a class-loading exception.

- Observation: Decision-side HTTP status extraction must treat invalid Turtle as an absent status, not as a routing failure.
  Evidence: A 2026-06-06 notebook run crashed in [default_escalation_controller.py](react_agent/ccrs/contingency/default_escalation_controller.py) while scanning the successful `http_post` body `Graph updated: http://127.0.1.1:8080/cells/0` for consecutive HTTP errors. [http_status.py](react_agent/ccrs/contingency/http_status.py) now catches parse errors as value-level absence; a focused smoke returned `None` for the non-RDF success body and `403` for an RDF/Turtle error body.
...
- Decision: Keep HTTP API error response escalation in Package C, not Package B.
  Rationale: Package B can expose RDF error bodies and interaction outcomes from normal LangGraph messages, but deciding that five consecutive HTTP status codes at or above 400 should trigger contingency CCRS is a graph-control policy. That policy belongs in [default_escalation_controller.py](react_agent/ccrs/contingency/default_escalation_controller.py) as an OR condition beside repeated tool invocation failures.
  Date/Author: 2026-05-31 / User direction and Codex

- Decision: Treat the `escalate_to_contingency_ccrs` tool description as an agent-facing control contract.
  Rationale: The LLM decides whether to call the tool from its name, schema, and description. The contract should explain operational triggers, required fields, and graph consequences in the agent's terms, without assuming the reader knows the ongoing CCRS design discussion.
  Date/Author: 2026-05-31 / User direction and Codex
...
### Package C: Escalation And Graph Routing (Complete First Pass; Reopened For Escalation Hardening)
...
The reopened Package C hardening keeps that separation of concerns but broadens the default controller's conservative policy. The default controller should escalate when either existing repeated tool invocation failures are detected or the latest run of normal tool responses shows a configurable number of consecutive HTTP API errors. The HTTP condition should inspect normal `ToolMessage` values, not a new custom history channel. It should recognize status codes at or above 400 from generic response metadata when available and from parseable message content when the tool body carries status information, including RDF error bodies from MaSE. The default threshold should be five consecutive HTTP API errors, with a graph/API/CLI configuration path if the existing controller configuration does not already provide a clean place for that threshold.
...
The tool contract in [escalation.py](react_agent/ccrs/contingency/escalation.py) must be written for the agent that sees the tool list. The name, docstring, and Pydantic field descriptions should explain when to use it: when repeated normal actions are not making progress, the agent is blocked or uncertain, observations contradict the current plan, an intended next resource cannot be accessed, or ordinary retrying is likely to waste cycles. The contract should also explain consequences: calling the tool asks the graph to pause normal tool execution for that cycle, invoke contingency CCRS, and return a recovery suggestion in subsequent CCRS prompt context. Field descriptions should tell the agent how to fill `type`, `trigger`, `current_resource`, `target_resource`, `failed_action`, and `reason` in scenario-neutral terms.
...
- [x] Rewrite the `escalate_to_contingency_ccrs` tool docstring and field descriptions so the agent can infer when to call it and what happens afterward.
- [x] Add focused smoke checks for the default controller: below-threshold HTTP errors do not escalate, five consecutive HTTP `>= 400` messages escalate, explicit LLM escalation still takes precedence, and repeated tool invocation failures still work.
...
The reopened Package C hardening is acceptable when focused controller checks show all of the following: fewer than five consecutive HTTP API error responses do not escalate by default; five consecutive `ToolMessage` responses with status code `>= 400` escalate with a distinct HTTP-error trigger; repeated tool invocation failures still escalate through their existing trigger; explicit `escalate_to_contingency_ccrs` tool calls still take precedence over controller-derived escalation; and the tool schema/docstring describes when to call it and the graph consequence without relying on external CCRS design context.
...
2026-05-31: Reopened Package C for escalation hardening after notebook-run inspection showed two gaps: the default controller only covers explicit escalation and tool invocation failures, while normal HTTP API error responses can still indicate being stuck; and the `escalate_to_contingency_ccrs` tool contract is too implicit for the agent to reliably choose it.

2026-05-31: Implemented the reopened Package C hardening. [http_status.py](react_agent/ccrs/contingency/http_status.py) extracts HTTP status codes from tool-message metadata, JSON content, and RDF/Turtle error bodies; [default_escalation_controller.py](react_agent/ccrs/contingency/default_escalation_controller.py) escalates after configurable consecutive HTTP API errors with a distinct trigger; [escalation.py](react_agent/ccrs/contingency/escalation.py) now describes `escalate_to_contingency_ccrs` in agent-facing terms; and [main.py](main.py) exposes `--contingency-http-error-threshold`.
