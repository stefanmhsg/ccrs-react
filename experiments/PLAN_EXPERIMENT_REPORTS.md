...
- [x] Enrich `actions.csv` with HTTP API response evidence. Current historical rows still only prove invocation, but fresh runs now log structured `[TOOL_NODE] Tool result: {...}` lines after tool execution with `tool_call_id`, `tool_name`, `target`, HTTP `status_code`, response length, and error metadata; the parser joins those fields into `actions.csv`.
...
Outcome and notes: Initial parser implemented 2026-06-07. It writes batch-level `runs.csv`, `agents.csv`, `mase-events.csv`, `mase-agent-moved.csv`, `mase-transactions.csv`, `cycle-durations.csv`, `decisions.csv`, `contingency.csv`, `opportunistic.csv`, `actions.csv`, `move-action-correlation.csv`, `java-library-evidence.csv`, `path-analysis-inputs/*.cells.txt`, and `summary.json`. Validation: syntax checks passed. The parser now filters MASE rows to `run.json` `agentName` / `agentNames` using BDI-style resource-label matching, so infrastructure agents from full viewer exports are excluded. Regenerating `react-ccrs-mazeV1-03-latest` produced 87 filtered MASE events, 40 movement rows, 47 transactions, 1 agent row, 625 contingency rows, 560 Java evidence rows, and 0 decision rows because the source log predates `react.ccrs.opportunistic.selection`. For `react-ccrs-mazeV1-01-latest`, regenerated artifacts contain 679 invoked tool actions, 120 valid MASE moves, and 120 move-action correlation rows; historical status fields are blank because that log predates structured tool-result logging. Remaining WP3 work: decide whether per-run CSV copies are useful before WP4 report generation.
